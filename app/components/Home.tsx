...
export default function Home({ lang }: HomeProps) {
        ...
        if (blockElement) {
          ...
          if (target && document.activeElement !== target) {
            ...
            if (
              ...
              (target as any).setSelectionRange
            ) {
  ...
  useEffect(() => {
    const onDocClick = (e: MouseEvent) => {
      ...
      if (current) {
        ...
        const normalizedTag = editingTag
          .trim()
          .replace(/\s+/g, "-")
          .toUpperCase();
      ...
      const emojis = symbols
        .filter(
          (s) =>
            ...
            removeAccents(s.description.en.main.toLowerCase()).includes(
              normalizedSearch,
            ) ||
            removeAccents(s.description.es.main.toLowerCase()).includes(
              normalizedSearch,
            ) ||
            s.description.en.secondary?.some((sec) =>
              removeAccents(sec.toLowerCase()).includes(normalizedSearch),
            ) ||
            s.description.es.secondary?.some((sec) =>
              removeAccents(sec.toLowerCase()).includes(normalizedSearch),
            ) ||
            s.tags?.en.some((tag) =>
              removeAccents(tag.toLowerCase()).includes(normalizedSearch),
            ) ||
            s.tags?.es.some((tag) =>
              removeAccents(tag.toLowerCase()).includes(normalizedSearch),
            ),
  ...
  const handleFileUpload = (blockId: string, files: FileList | null) => {
    ...
    filesToProcess.forEach((file) => {
      ...
      if (isImage) {
        ...
        const totalFiles = currentAttachments.filter(
          (a) =>
            !a.type.startsWith("image/") &&
            !a.name.toLowerCase().endsWith(".jpg") &&
            !a.name.toLowerCase().endsWith(".jpeg"),
  ...
  const handlePaste = (e: React.ClipboardEvent, blockId: string) => {
    ...
    if (text) {
      const isUrl = /^(https?:\/\/|www\.)[\w\-.:/?#@!$&'()*+,;=%~]+$/i.test(
        text.trim(),
      );
      ...
      if (
        ...
        if (start !== null && end !== null && start !== end) {
          ...
          if (inputType === "content") {
            ...
            const newText = `[${selectedText}](${text.trim()})`;
            const updated =
              editingTitle.substring(0, start) +
              newText +
              editingTitle.substring(end);
            setEditingTitle(updated);
          } else if (inputType === "tag") {
            const selectedText = editingTag.substring(start, end);
            const newText = `[${selectedText}](${text.trim()})`;
            const updated =
              editingTag.substring(0, start) +
              newText +
              editingTag.substring(end);
  ...
  const insertEmoji = (emoji: SymbolItem) => {
    ...
    const after = editingContent.slice(
      editingContent.indexOf(":", triggerIdx) + 1 + emojiSearch.length,
    );
    ...
    if (showEmojiPicker && filteredEmojis.length > 0) {
      ...
      if (e.key === "ArrowUp") {
        ...
        setEmojiSelectedIndex(
          (prev) => (prev - 1 + filteredEmojis.length) % filteredEmojis.length,
        );
        return;
      }
  ...
  const formatText = (text: string) => {
    ...
    formatted = formatted.replace(mdLinkRegex, (_, text, url) => {
      ...
      mdLinks.push(
        `<a href="${fullUrl}" target="_blank" rel="noopener noreferrer" class="font-bold text-[#6866D6] hover:underline">${text}</a>`,
      );
  ...
  const availableTags = Array.from(
    new Set(blocks.map((b) => b.userTag).filter(Boolean)),
  ) as string[];
          ...
          <div className="flex flex-wrap justify-center gap-2 mb-0 animate-in fade-in slide-in-from-top-2 duration-500">
            {availableTags.map((tag) => (
              <button
                ...
                className={`group relative px-3 py-1 text-xs font-bold rounded-full border cursor-pointer transition-all flex items-center gap-2 uppercase ${
                  selectedTag === tag
              ...
              >
                {selectedTag === tag ? (
                  ...
                  <span
                    ...
                    style={{ backgroundColor: tagColors[tag] || "#FEFCE8" }}
                  />
                )}
                #{tag}
              </button>
          ...
          <React.Fragment key={block.id}>
            ...
            >
                  ...
                  {editingBlockId === block.id && (
                    <button
                      ...
                      aria-label={
                        deletingBlockId === block.id
                          ? t.ariaDelete
                          : `${t.ariaDeleteSpecific} ${block.title}`
                      }
                    >
                      {deletingBlockId === block.id ? (
                        <Check className="w-4 h-4" />
                          ...
                          >
                            ...
                            <div className="flex flex-col">
                              <span className="font-medium">
                                {item.description[lang].main}
                              </span>
                              <span className="text-xs opacity-60">
                                :
                                {item.description.en.main
                                  .toLowerCase()
                                  .replace(/\s+/g, "_")}
                                :
                              </span>
                      ...
                      return (
                        ...
                        >
                          <img
                            src={img.url}
                            alt=""
                            className="w-full h-full object-cover transition-transform group-hover/img:scale-105 pointer-events-none"
                          />
                          ...
                          {editingBlockId === block.id && (
                            <button
                              ...
                              className={`absolute top-1 right-1 p-1 rounded-full transition-all cursor-pointer shadow-sm z-10 border active:scale-95 ${
                                isDeletingThisFile
                    ...
                    <div className="flex items-center gap-3 px-2 py-1 bg-zinc-800 rounded-md order-1 sm:order-none scale-90 sm:scale-100 origin-left">
                      <div className="flex items-center gap-1 border-r border-zinc-700 pr-3">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setEditingTag(e.target.value.toUpperCase())
                          }
                      ...
                      <div className="flex items-center gap-1">
                        {[
                          {
                            h: "#FEFCE8",
                            n: lang === "es" ? "Amarillo" : "Yellow",
                          },
                          { h: "#FEE2E2", n: lang === "es" ? "Rojo" : "Red" },
                          { h: "#E0F2FE", n: lang === "es" ? "Azul" : "Blue" },
                          {
                            h: "#DCFCE7",
                            n: lang === "es" ? "Verde" : "Green",
                          },
                          {
                            h: "#F3E8FF",
                            n: lang === "es" ? "Violeta" : "Purple",
                          },
                          {
                            h: "#FFEDD5",
                            n: lang === "es" ? "Naranja" : "Orange",
                          },
                        ].map((c) => (
                          <button
                            key={c.h}
                        ...
                        return (
                          ...
                          >
                            <button
                              ...
                              className={`flex items-center gap-2 px-3 py-1.5 rounded-md transition-all scale-90 sm:scale-100 origin-right cursor-pointer shadow-sm border max-w-[150px] sm:max-w-[200px] group/btn ${
                                editingBlockId === block.id
                                  ? isDeletingThisFile
                            ...
                            >
                              {editingBlockId === block.id ? (
                                ...
                                <Paperclip
                                  size={14}
                                  className="text-zinc-400 flex-shrink-0 pointer-events-none"
                                />
                              )}
              ...
              <input
                ...
                accept={(() => {
                  ...
                  const currentAttach = (block.attachments || []).filter(
                    (a) =>
                      a.type.startsWith("image/") ||
                      a.name.toLowerCase().endsWith(".jpg") ||
                      a.name.toLowerCase().endsWith(".jpeg"),
            ...
            {blocks.length > 1 && (
              <div className="absolute left-full ml-3 bottom-[-0.24px] flex flex-col gap-1 lg:opacity-0 lg:group-hover/note:opacity-100 transition-opacity">
                <button
                  ...
                  className="text-gray-400 hover:text-[#6866D6] transition-all cursor-pointer disabled:opacity-30 disabled:cursor-default p-1 bg-white/50 rounded shadow-sm hover:shadow border border-black/5"
            ...
            {idx === filteredBlocks.length - 1 && (
              <div className="flex justify-center mt-4">
                <button
                  onClick={addBlock}
                  className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 bg-transparent border-none outline-none cursor-pointer w-full max-w-lg"
                  style={{ height: "32px" }}
                  aria-label={t.addBlock || "Agregar nota"}
                >
                  <div className="mx-auto w-1/2 h-1 rounded-full bg-zinc-300 hover:bg-[#6866D6] transition-colors" />
                </button>
              </div>
            )}
          </React.Fragment>
